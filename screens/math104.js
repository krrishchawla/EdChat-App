...
const incrementVotes = async (id) => {
    try {
      // Get the current value of votes for math104
      ...
      const currentVotes = classData.votes;
      ...
      console.log(`Successfully incremented votes for math104. New value: ${updatedData.votes}`);
    } catch (error) {
      console.error(error.message);
    }
  }
  
const decrementVotes = async (id) => {
    try {
      // Get the current value of votes for math104
      const { data: classData, error } = await supabase
        .from('main')
...
export default function Math104({navigation}) {
  ...
  React.useEffect(() => {
    async function fetchFilteredData() {
      const { data, error } = await fetchDataByClassName('math104');
      if (error) {
        console.error(error);
      } else {
        setData(data);
    ...
    <SafeAreaView>
      <View style={styles.body}>
        <View style={styles.headerView}>
            ...
            <Text style={styles.buttonText}>MATH 104</Text>
